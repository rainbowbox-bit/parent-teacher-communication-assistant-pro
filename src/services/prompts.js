...
【專業賦能】：請在回覆中展現對孩子的細膩觀察，並**務必**在回覆的最末端（結尾貼心話之後），加上一行：(建議附照：[請AI建議一張最具說服力的照片內容])。
`;
...
**任務：** 扮演資深幼教老師，將情境轉化為極致人性化、溫暖且專業的繁體中文回覆。重點在於「同理家長」與「描述孩子具體狀況」，而不是背誦課綱理論。請使用「純文字」輸出，像在打 LINE 一樣自然分段即可。
---
...
export function buildPolishPrompt(originalText, type) {
  const instructions = {
    soften: '請將這段回覆修改得**更委婉、更溫柔、更有同理心**，多使用緩衝詞，讓焦慮的家長聽了能感到被安撫。保持原意不變。',
    firm:   '請將這段回覆修改得**更堅定、更專業、界線更分明**，去除過多的情感贅字，強調幼兒園的規範與集體利益。保持原意不變。',
